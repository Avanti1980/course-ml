...
\tikzset{nc/.style={base,circle,minimum width=32,fill=SolarizedBase2}}
...
\begin{tikzpicture} [scale=0.78]
    ...
    \node [nc,fill=SolarizedBase1] (concatenate1) at (\r-3,-3.5) {$+$};
    ...
    \node [nc,fill=SolarizedBase1] (concatenate2) at (\r+2.5,-5) {$+$};
    ...
    \draw [dashed,very thick,SolarizedBlue] (\r-4.2,-6) -- (\a+4.5,-6) -- (\a+4.5,5) -- (\r-4.2,5) -- cycle;

    \node [nc,label={[inner sep=6pt,SolarizedBase01]right:\large 向量元素乘}] at (\r,-7.5) {$\times$};

    \node [nc,fill=SolarizedBase2,label={[inner sep=6pt,SolarizedBase01]right:\large 向量和}] at (\z,-7.5) {$+$};

    \node [nc,fill=SolarizedBase1,label={[inner sep=6pt,SolarizedBase01]right:\large 向量拼接}] at (\a-1,-7.5) {$+$};
