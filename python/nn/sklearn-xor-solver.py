...
h_array = [2, 3, 4, 5]
act_array = [['identity', '恒等激活'], ['logistic', '对率激活'], ['tanh', '双曲正切激活'], ['relu', 'ReLU激活']]
solver_array = [['lbfgs', 0], ['sgd', 0], ['sgd', 0.9], ['adam', 0]]

col = len(solver_array)
...
with plt.style.context('Solarize_Light2'):
    ...
    for i, (sol, mom) in enumerate(solver_array):
        ...
        if sol == 'sgd':
            ax.set_title(f'{sol}({mom})，准确率 = {acc:.2f}', size=18)
        else:
            ax.set_title(f'{sol}，准确率 = {acc:.2f}', size=18)
