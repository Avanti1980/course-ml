...
low_x, high_x, low_y, high_y = -0.15, 0.1, -0.1, 0.15
xx, yy = np.meshgrid(np.linspace(low_x, high_x, 500), np.linspace(low_y, high_y, 500))

zz_l1 = np.array([np.abs(xi) + np.abs(yi) for xi, yi in zip(np.ravel(xx), np.ravel(yy))])  # l1 范数
zz_l2 = np.array([xi**2 + yi**2 for xi, yi in zip(np.ravel(xx), np.ravel(yy))])  # l2 范数
zz_ls = np.array([lsobj(X, y.reshape(-1, 1), np.array([t0, t1]).reshape(-1, 1))
                  for t0, t1 in zip(np.ravel(xx), np.ravel(yy))])
...
with plt.style.context('Solarize_Light2'):
    ...
    ax[0].contour(xx, yy, Z_l1, levels=levels_l1)
    ax[0].contour(xx, yy, Z_ls, levels=levels_ls)
    ax[1].contour(xx, yy, Z_l2, levels=levels_l2)
    ax[1].contour(xx, yy, Z_ls, levels=levels_ls)

    plt.savefig("sparse-norm.svg")
