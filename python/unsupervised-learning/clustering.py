...
with plt.style.context('Solarize_Light2'):
    ...
    for i, n_components in enumerate([1, 2]):
        ...
        axs[i] = sns.scatterplot(x=X[:, 0], y=X[:, 1], c=k_means.labels_, cmap='inferno', legend=False, alpha=0.5, s=10, ax=axs[i])  # 样本
        axs[i] = sns.scatterplot(x=k_means.cluster_centers_[:, 0], y=k_means.cluster_centers_[:, 1], c=range(n_clusters), cmap='inferno', s=30, legend=False, marker='D', ax=axs[i])  # 簇中心
