...
$$
\begin{align*}
    \min_{w_j} ~ g (w_j) = \frac{1}{2} \sum_{i \in [m]} \left( \sum_{j=0}^n w_j x_i^j - y_i \right)^2
\end{align*}
$$
...
$$
\begin{align*}
    \frac{\partial g}{\partial w_j} = \sum_{i \in [m]} \left( \sum_{j=0}^n w_j x_i^j - y_i \right) x_i^j
\end{align*}
$$
...
- 训练不足时，模型还很粗糙，拟合能力不强，偏差占主导
- 训练程度加深后，模型开始捕捉数据集的细节，方差占主导

<img src="../tikz/bias-var-dec.svg" class="center top5 width45" title="随机生成了 5 个数据集：1 阶多项式高偏差、低方差；30 阶多项式低偏差、高方差；4 阶多项式低偏差、低偏差，是最理想的模型">
