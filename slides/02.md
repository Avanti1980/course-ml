...
<p class="left10">$\Pr (\text{约会}|\text{次序},\text{时间},\ldots,\text{电视}) = \frac{\Pr(\text{次序},\text{时间},\ldots,\text{电视}|\text{约会}) ~ \Pr(\text{约会}) \quad \quad \quad ~~~~~}{\Pr(\text{次序},\text{时间},\ldots,\text{电视}) \qquad \quad}$</p>

<div class="top2"></div>

- {==先验==}$\Pr (\text{约会})$反映了在没有任何信息的情况下对约会的信念
- 数据通过{==似然==}$\Pr (\text{次序},\text{时间},\ldots,\text{电视}|\text{约会})$调整我们对约会的信念
