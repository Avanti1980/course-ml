...
给定模型$f$、样例集$D = \{ (\xv_i, y_i) \}_{i \in [m]}$
...
给定模型$f$、训练样例集$D = \{ (\xv_i, y_i) \}_{i \in [m]}$，其中$\xv_i \sim \Dcal$
...
$$
\begin{align*}
    E(f;\Dcal) = \int_{\xv \sim \Dcal} (f(\xv) - y)^2 \Pr(\xv) \diff \xv
\end{align*}
$$
...
$$
\begin{align*}
    E(f;\Dcal) = \int_{\xv \sim \Dcal} \Ibb(f(\xv) \ne y) \Pr(\xv) \diff \xv
\end{align*}
$$
