...
\begin{align*}
    ...
    & \max_{\alphav \ge \zerov} ~ \left\{ - \frac{1}{2} \sum_{i \in [m]} \sum_{j \in [m]} \alpha_i \alpha_j y_i y_j \underbrace{\phi(\xv_i)^\top \phi(\xv_j)}_{= ~ \kappa(\xv_i,\xv_j)} + \sum_{i \in [m]} \alpha_i \right\}, \quad \st ~ \yv^\top \alphav = 0
\end{align*}
$$
...
\begin{align*}
    \quad & \min_{\wv,b} ~ \frac{1}{2} \|\wv\|_2^2, \quad \st ~ y_i (\wv^\top \phi(\xv_i) + b) \ge 1, ~ \forall i \in [m] \\
    & \max_{\alphav \ge \zerov} ~ \left\{ - \frac{1}{2} \sum_{i \in [m]} \sum_{j \in [m]} \alpha_i \alpha_j y_i y_j \kappa(\xv_i,\xv_j) + \sum_{i \in [m]} \alpha_i \right\}, \quad \st ~ \yv^\top \alphav = 0
\end{align*}
$$
