...
我的批注 $\Ebb[y|\xv]$和$\mathop{\arg\max}_{\yhat \in [c]} \Pbb(\yhat|\xv)$均依赖未知的$\Pbb$，无法直接得到
...
利用训练集求$\mathop{\arg\max}_{y \in [c]} \Pbb(y|\xv)$有两种思路

判别式方法：用线性判别式直接拟合$\Pbb(y|\xv)$，如对率回归

- 二分类：$\Pbb(y=1|\xv) = \sigma(\wv^\top \xv)$
- 多分类：$[\Pbb(y=1|\xv), \ldots, \Pbb(y=c|\xv)] = \softmax (\wv_1^\top \xv, \ldots, \wv_c^\top \xv)$
...
问题：如何用训练集估计$\Pbb(y), ~ \Pbb(x_1 | y), ~ \Pbb(x_2 | y), ~ \ldots, ~ \Pbb(x_d | y)$？
...
对于$\Pbb(y)$，记参数$\alpha_k = \Pbb(y = k)$，于是
...
对于$\Pbb(\xv | y)$，设第$j$个特征共有$n_j$种不同取值$v_1^{(j)}, \ldots, v_{n_j}^{(j)}$
...
$$
\begin{align*}
    \quad & \Pbb(x_j | y = k, \thetav) = \prod_{l \in [n_j]} \theta_{kjl}^{\Ibb(x_j = v_l^{(j)})}, \quad \sum_{l \in [n_j]} \theta_{kjl} = 1
\end{align*}
$$
...
\begin{align*}
    ...
    & = \sum_{k \in [c]} \sum_{i \in [m]} \sum_{j \in [d]} \Ibb(y^{(i)}=k) \ln \Pbb(x_j^{(i)} | y^{(i)} = k, \thetav) \quad \longleftarrow \text{特征独立性} \\
    & = \sum_{k \in [c]} \sum_{i \in [m]} \sum_{j \in [d]} \Ibb(y^{(i)}=k) \ln \prod_{l \in [n_j]} \Pbb( x_j^{(i)} = v_l^{(j)} | y^{(i)}=k)^{\Ibb(x_j^{(i)} = v_l^{(j)})} \\
...
\begin{align*}
    ...
    & \quad \Pbb (\text{选取词}v_j | y = k) = \frac{\text{第}k\text{类文本中词}v_j\text{出现总次数} + \lambda \quad~~~}{\text{第}k\text{类文本的总词数} + d \lambda}
\end{align*}
$$
