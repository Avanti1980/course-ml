...
$$
\begin{align*}
    \quad R_{\Dcal} (h) & = \Ebb_{(\xv,y) \sim \Dcal} [\ell(y, h(\xv))] = \iint \ell(y, h(\xv)) \Pbb(\xv, y) \diff \xv \diff y \\
...
$$
\begin{align*}
    \quad h^\star(\xv) = \mathop{\arg\min}_{h(\xv)} \int \ell(y, h(\xv)) \Pbb(y|\xv) \diff y
\end{align*}
$$
...
$$
\begin{align*}
    \quad \Ebb_{(\xv,y) \sim \Dcal} [(y - h(\xv))^2] = \Ebb_{\xv} [(h(\xv) - \Ebb [y|\xv])^2] + \noise
\end{align*}
$$
...
$$
\begin{align*}
    \quad h^\star(\xv) & = \mathop{\arg\min}_{\yhat \in [c]} \sum_{y \in [c]} \Ibb(y \ne \yhat) \Pbb(y|\xv) \\[4pt]
...
我的启示 对于分类问题，{==泛化风险最小化==}等价于{==后验概率最大化==}
...
$$
\begin{align*}
    \quad \Pbb(\xv | y) = \Pbb(x_1 | y) \Pbb(x_2 | x_1, y) \cdots \Pbb(x_d | x_{d-1}, \ldots, x_2, x_1, y)
\end{align*}
$$
...
$$
\begin{align*}
    \quad \Pbb(\xv | y) = \Pbb(x_1 | y) \Pbb(x_2 | y) \cdots \Pbb(x_d | y) = \prod_{j \in [d]} \Pbb(x_j | y)
\end{align*}
$$
...
$$
\begin{align*}
    \quad \Pbb(y | \xv) = \Pbb(y) \Pbb(x_1 | y) \Pbb(x_2 | y) \cdots \Pbb(x_d | y)
\end{align*}
$$
...
$$
\begin{align*}
    \quad \Pbb(y | \alphav) = \prod_{k \in [c]} \Pbb(y = k)^{\Ibb(y=k)} = \prod_{k \in [c]} \alpha_k^{\Ibb(y=k)}
\end{align*}
$$
...
记参数$\theta_{kjl} = \Pbb( x_j = v_l^{(j)} | y=k)$，于是对$\forall k \in [c]$和$\forall j \in [d]$有

$$
\begin{align*}
    \quad & \Pbb(x_j | y = k, \thetav) = \prod_{l \in [n_j]} \Pbb( x_j = v_l^{(j)} | y=k)^{\Ibb(x_j = v_l^{(j)})} = \prod_{l \in [n_j]} \theta_{kjl}^{\Ibb(x_j = v_l^{(j)})} \\
    & \sum_{l \in [n_j]} \theta_{kjl} = 1
\end{align*}
$$
...
其中$B_{kjl}$是第$k$类样本中第$j$个特征取值$v_l^{(j)}$的样本数
...
\begin{align*}
    \quad \mathrm{LL} (\thetav) & = \sum_{k \in [c]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) \ln \Pbb (\xv^{(i)} | y^{(i)} = k, \thetav) \\
    & = \sum_{k \in [c]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) \ln \left\{ \frac{(x_1 + \cdots + x_d)!}{x_1! \cdots x_d!} \prod_{j \in [d]} \theta_{kj}^{x_j^{(i)}} \right\} \\
...
\begin{align*}
    ...
    & B_{kj} = \sum_{i \in [m]} \Ibb(y^{(i)}=k) x_j^{(i)} = \text{词}v_j\text{在第}k\text{类文本中出现总次数}
\end{align*}
$$
...
$$
\begin{align*}
    \quad \frac{\partial L}{\partial \theta_{kj}} = \frac{B_{kj}}{\theta_{kj}} - \lambda = 0 \Longrightarrow \theta_{kj} = \frac{\text{词}v_j\text{在第}k\text{类文本中出现总次数}\quad~~~}{\text{第}k\text{类文本的总词数}}
\end{align*}
$$
...
$$
\begin{align*}
    & \quad \Pbb (y = k) = \frac{\text{第}k\text{类样本数} + \lambda ~~~~}{\text{总样本数} + c \lambda} \\[6pt]
    & \quad \Pbb ( x_j = v_l^{(j)} | y=k) = \frac{\text{第}k\text{类样本中第}j\text{个特征取值}v_l^{(j)}\text{的样本数} + \lambda \qquad ~}{\text{第}k\text{类样本数} + n_j \lambda} \\[6pt]
    & \quad \Pbb (x_j = 1 | y = k) = \frac{\text{第}k\text{类文本中包含词}v_j\text{的文本数} + \lambda \quad ~~~}{\text{第}k\text{类文本数} + d \lambda} \\[6pt]
    & \quad \Pbb (\text{选取词}v_j | y = k) = \frac{\text{词}v_j\text{在第}k\text{类文本中出现总次数} + \lambda \quad~~~}{\text{第}k\text{类文本的总词数} + d \lambda}
\end{align*}
$$
