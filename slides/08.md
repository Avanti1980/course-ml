...
<div class="top-60per left58per fs16">
...
$$
\begin{align*}
    \qquad [\theta_{1k}; \ldots; \theta_{jk}; \ldots; \theta_{dk}] \in \Delta_d, \quad \Pbb (\xv | y = k, \thetav) = \frac{(x_1 + \cdots + x_d)!}{x_1! \cdots x_d!} \prod_{j \in [d]} \theta_{jk}^{x_j}
\end{align*}
$$
...
$\Fbb = \Rbb$，词频 - 逆文本频率模型，$x_j \sim \Ncal(\mu_{jk}, \sigma_{jk}^2)$
...
剩下只需用极大似然估计$\alphav, \thetav, \muv, \sigmav$
...
$$
\begin{align*}
    \qquad \theta_{jk} = \frac{B_{jk}}{B_{jk} + \Bbar_{jk}} = \frac{\text{第}k\text{类样本中包含词}v_j\text{的文本数}\quad ~~~}{\text{第}k\mbox{类文本数}}
\end{align*}
$$
...
$$
\begin{align*}
    \qquad \mathrm{LL} (\thetav) & = \sum_{k \in [c]} \sum_{j \in [d]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) x_j^{(i)} \log \theta_{jk} = \sum_{k \in [c]} \sum_{j \in [d]} B_{jk} \log \theta_{jk}
\end{align*}
$$
...
拉格朗日函数$L = \sum_{j \in [d]} B_{jk} \log \theta_{jk} - \lambda ( \sum_{j \in [d]} \theta_{jk} - 1 )$
...
$$
\begin{align*}
    \qquad \mathrm{LL} (\muv, \sigmav) = \sum_{k \in [c]} \sum_{j \in [d]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) \left( - \frac{(x_j^{(i)} - \mu_{jk})^2}{2 \sigma_{jk}^2} - \log \sigma_{jk}) \right)
\end{align*}
$$
...
$\Pr(\text{时间}=\text{周六}|\text{约会}=\text{是})=3/8$
$\Pr(\text{方式}=\text{逛街}|\text{约会}=\text{是})=3/8$
$\Pr(\text{天气}=\class{blue}{\text{雨天}}|\text{约会}=\text{是})=\class{blue}{0/8}$
$\Pr(\text{课业}=\text{适中}|\text{约会}=\text{是})=1/8$
$\Pr(\text{疫情}=\text{清零}|\text{约会}=\text{是})=5/8$
$\Pr(\text{电视}=\text{无聊}|\text{约会}=\text{是})=2/8$
...
$\Pr(\text{时间}=\text{周六}|\text{约会}=\text{否})=3/9$
$\Pr(\text{方式}=\text{逛街}|\text{约会}=\text{否})=4/9$
$\Pr(\text{天气}=\class{blue}{\text{雨天}}|\text{约会}=\text{否})=\class{blue}{2/9}$
$\Pr(\text{课业}=\text{适中}|\text{约会}=\text{否})=4/9$
$\Pr(\text{疫情}=\text{清零}|\text{约会}=\text{否})=2/9$
$\Pr(\text{电视}=\text{无聊}|\text{约会}=\text{否})=3/9$
...
$\Large \frac{8}{17} \frac{3}{8} \frac{3}{8} \class{blue}{\frac{0}{8}} \frac{1}{8} \frac{5}{8}  \frac{2}{8} = 0$
