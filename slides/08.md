...
$$
\begin{align*}
    \quad \mathrm{LL} & = \ln \Pbb(D | \alphav, \thetav) = \sum_{i \in [m]} \ln \Pbb(\xv^{(i)}, y^{(i)} | \alphav, \thetav) \\
    & = \sum_{i \in [m]} \ln \prod_{k \in [c]} \Pbb(\xv^{(i)}, y^{(i)} = k | \alphav, \thetav)^{\Ibb(y^{(i)}=k)} \\
    & = \sum_{i \in [m]} \sum_{k \in [c]} \Ibb(y^{(i)}=k) \ln \Pbb(\xv^{(i)}, y^{(i)} = k | \alphav, \thetav) \\
    & = \sum_{i \in [m]} \sum_{k \in [c]} \Ibb(y^{(i)}=k) \ln \Pbb(y^{(i)} = k | \alphav) \\ & \qquad \qquad \qquad + \sum_{i \in [m]} \sum_{k \in [c]} \Ibb(y^{(i)}=k) \ln \Pbb(\xv^{(i)} | y^{(i)} = k, \thetav) \\[4pt]
    & = \sum_{k \in [c]} \sum_{i \in [m]} \underbrace{\Ibb(y^{(i)}=k) \ln \alpha_k}_{\alphav~\text{相关的项}\qquad} + \sum_{k \in [c]} \sum_{i \in [m]} \underbrace{\Ibb(y^{(i)}=k) \ln \Pbb(\xv^{(i)} | y^{(i)} = k, \thetav)}_{\thetav~\text{相关的项}\qquad}
\end{align*}
$$
...
$$
\begin{align*}
    \qquad \max_{\alpha_k} ~ \sum_{k \in [c]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) \ln \alpha_k = \sum_{k \in [c]} A_k \ln \alpha_k, \quad \st ~ \sum_{k \in [c]} \alpha_k = 1
\end{align*}
$$

拉格朗日函数$L = \sum \limits_{k \in [c]} A_k \ln \alpha_k - \lambda ( \sum \limits_{k \in [c]} \alpha_k - 1 )$
...
\begin{align*}
    ...
    & = \sum_{k \in [c]} \sum_{i \in [m]} \sum_{j \in [d]} \Ibb(y^{(i)}=k) \ln \Pbb(x_j^{(i)} | y^{(i)} = k, \thetav) \\
    & = \sum_{k \in [c]} \sum_{i \in [m]} \sum_{j \in [d]} \Ibb(y^{(i)}=k) \ln \prod_{l \in [n_j]} \Pbb( x_j^{(i)} = v_l^{(j)} | y=k)^{\Ibb(x_j^{(i)} = v_l^{(j)})} \\
    & = \sum_{k \in [c]} \sum_{j \in [d]} \sum_{l \in [n_j]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) \Ibb(x_j^{(i)} = v_l^{(j)}) \ln \theta_{jlk} \\
    & = \sum_{k \in [c]} \sum_{j \in [d]} \sum_{l \in [n_j]} B_{jlk} \ln \theta_{jlk}
\end{align*}
$$
...
$$
\begin{align*}
    \quad \max_{\theta_{jlk}} ~ \sum_{l \in [n_j]} B_{jlk} \ln \theta_{jlk}, \quad \st ~ \sum_{l \in [n_j]} \theta_{jlk} = 1
\end{align*}
$$

拉格朗日函数$L = \sum \limits_{l \in [n_j]} B_{jlk} \ln \theta_{jlk} - \lambda ( \sum \limits_{l \in [n_j]} \theta_{jlk} - 1 )$
...
$$
\begin{align*}
    \qquad \mathrm{LL} (\thetav) & = \sum_{k \in [c]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) \ln \Pbb (\xv^{(i)} | y^{(i)} = k, \thetav) \\
    & = \sum_{k \in [c]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) \ln \prod_{j \in [d]} \theta_{jk}^{x_j^{(i)}} (1 - \theta_{jk})^{1 - x_j^{(i)}}        \\
    & = \sum_{k \in [c]} \sum_{j \in [d]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) (x_j^{(i)} \ln \theta_{jk} + (1 - x_j^{(i)}) \ln (1 - \theta_{jk}) ) \\
    & = \sum_{k \in [c]} \sum_{j \in [d]} (B_{jk} \ln \theta_{jk} + \Bbar_{jk} \ln (1 - \theta_{jk}) )
\end{align*}
$$
...
$$
\begin{align*}
    \qquad \mathrm{LL} (\thetav) & = \sum_{k \in [c]} \sum_{j \in [d]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) (x_j^{(i)} \ln \theta_{jk} + (1 - x_j^{(i)}) \ln (1 - \theta_{jk}) ) \\
    & = \sum_{k \in [c]} \sum_{j \in [d]} (B_{jk} \ln \theta_{jk} + \Bbar_{jk} \ln (1 - \theta_{jk}) )
\end{align*}
$$
...
$$
\begin{align*}
    \qquad & \mathrm{LL} (\thetav) = \sum_{k \in [c]} \sum_{j \in [d]} (B_{jk} \ln \theta_{jk} + \Bbar_{jk} \ln (1 - \theta_{jk}) ) \\
...
$$
\begin{align*}
    \qquad \max_{\theta_{jk}} ~ \{ B_{jk} \ln \theta_{jk} + \Bbar_{jk} \ln (1 - \theta_{jk}) \}
\end{align*}
$$
...
\begin{align*}
    \qquad \mathrm{LL} (\thetav) & = \sum_{k \in [c]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) \ln \Pbb (\xv^{(i)} | y^{(i)} = k, \thetav) \\
    & = \sum_{k \in [c]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) \ln \left\{ \frac{(x_1 + \cdots + x_d)!}{x_1! \cdots x_d!} \prod_{j \in [d]} \theta_{jk}^{x_j^{(i)}} \right\} \\
    & = \const + \sum_{k \in [c]} \sum_{j \in [d]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) x_j^{(i)} \ln \theta_{jk}
\end{align*}
$$
...
$$
\begin{align*}
    \qquad \sum_{k \in [c]} \sum_{j \in [d]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) x_j^{(i)} \ln \theta_{jk} = \sum_{k \in [c]} \sum_{j \in [d]} B_{jk} \ln \theta_{jk}
\end{align*}
$$
...
$$
\begin{align*}
    \qquad \max_{\theta_{jk}} ~ \sum_{j \in [d]} B_{jk} \ln \theta_{jk}, \quad \st ~ \sum_{j \in [d]} \theta_{jk} = 1
\end{align*}
$$

<div class="top-4"></div>

拉格朗日函数$L = \sum \limits_{j \in [d]} B_{jk} \ln \theta_{jk} - \lambda ( \sum \limits_{j \in [d]} \theta_{jk} - 1 )$
...
$$
\begin{align*}
    \qquad \mathrm{LL} & (\muv, \sigmav) = \sum_{k \in [c]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) \ln \Pbb (\xv^{(i)} | y^{(i)} = k, \muv, \sigmav) \\
    & = \sum_{k \in [c]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) \ln \prod_{j \in [d]} \frac{1}{\sqrt{2\pi \sigma_{jk}^2}} \exp \left( - \frac{(x_j^{(i)} - \mu_{jk})^2}{2 \sigma_{jk}^2} \right) \\
    & = \const + \sum_{k \in [c]} \sum_{j \in [d]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) \left( - \frac{(x_j^{(i)} - \mu_{jk})^2}{2 \sigma_{jk}^2} - \ln \sigma_{jk}) \right)
\end{align*}
$$
...
$$
\begin{align*}
    \qquad \sum_{k \in [c]} \sum_{j \in [d]} \sum_{i \in [m]} \Ibb(y^{(i)}=k) \left( - \frac{(x_j^{(i)} - \mu_{jk})^2}{2 \sigma_{jk}^2} - \ln \sigma_{jk}) \right)
\end{align*}
$$
...
$$
\begin{align*}
    \qquad \min_{\sigma_{jk}} \sum_{i \in [m]} \Ibb(y^{(i)}=k) \left( \frac{(x_j^{(i)} - \mu_{jk})^2}{2 \sigma_{jk}^2} + \ln \sigma_{jk}) \right)
\end{align*}
$$
