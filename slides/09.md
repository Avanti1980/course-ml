...
- 未知参数$\theta = \Pbb(\text{正面})$，与硬币固有的
- 观测数据$X$为$t$次抛掷中有$k$次正面
...
- 若与似然相乘后，后验与先验属同一分布族，仅参数有变化，则该先验称为对应于该似然的{==共轭先验==} (conjugate prior)
- 贝塔分布是二项式分布的共轭先验
...
- 先验是对 (生成式) 模型的信念，$\Theta$是模型的参数
- 似然是在给定以$\Theta$为参数的生成机制的条件下，观测到$X$的可能性
- 后验是在得到观测数据$X$后，对模型参数$\Theta$的修正
...
- 先估计$\theta^{\text{MAP}}$，再计算$\Pbb(\xhat|\theta^{\text{MAP}})$，这样做忽略了$\theta$本身的随机性，尤其当后验$\Pbb(\theta|X)$是个{==平坦==}的分布时，只取一个点来做决策风险很大
...
| $\{ v_1, \ldots, v_n \}$ |     $\left. \mathrm{Cate}(\xv \right\arrowvert \thetav)$      | $\left. \mathrm{Dir}(\thetav \right\arrowvert \mv)$  | $\left. \mathrm{Dir}(\thetav \right\arrowvert m_1 + A_1, \ldots, m_c + A_c)$ |
|     $\Nbb$(词袋模型)     |     $\left. \mathrm{Mult}(\xv \right\arrowvert \thetav)$      | $\left. \mathrm{Dir}(\thetav \right\arrowvert \mv)$  | $\left. \mathrm{Dir}(\thetav \right\arrowvert m_1 + A_1, \ldots, m_c + A_c)$ |
|          $\Rbb$          | $\left. \Ncal(x_{kj} \right\arrowvert \mu_{kj}, \sigma_{kj})$ |          均值不固定、精度固定时，为高斯分布          |
|            -             |                               -                               |         均值固定、精度不固定时，为威沙特分布         |
