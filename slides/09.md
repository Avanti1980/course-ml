...
- 先验 (prior) 对应模型参数$\Theta$，例如科研水平伯努利分布参数$80\%$
- 证据 (evidence) 对应观测到的数据$X$
- 似然 (likelihood) 是在给定参数$\Theta$的条件下观测到数据$X$的概率
- 后验 (posterior) 是在得到观测数据$X$后，对模型参数$\Theta$的修正
...
- 通过先验可以很自然地引入{==领域知识==} (domain knowledge)
...
- 由于先验 (伪数据) 的存在，不会出现频率主义中$\theta^{\text{MAP}} = 1$的情况
