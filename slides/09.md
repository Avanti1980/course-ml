...
- 马氏距离$\dist_\Mv (\xv, \zv) \triangleq \sqrt{(\xv - \zv)^\top \Mv (\xv - \zv)}$，$\Mv=\diag\{w_1, \ldots, w_d\}$时，马氏距离就是加权平方距离$\sqrt{\sum_{j \in [d]} w_j (x_j - z_j)^2}$
...
由{==可分性==}，$\Xcal$存在可数稠密子集$\Acal$，且存在点$\av \in B_\xv(r/3) \wedge \Acal$，考虑其邻域$B_\av (r/2)$，显然$\xv \in B_\av (r/2)$，又$B_\av (r/2) \subseteq B_\xv(r)$，故$\Pbb(B_\av (r/2)) = 0$
