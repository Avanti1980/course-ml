...
\title[Machine Learning]{LN3. K-Nearest Neighbors} % The short title appears at the bottom of every slide, the full title is only on the title page
...
\date{2021年09月24日} % Date, can be changed to a custom date
%====================================================
\frame{\titlepage}
...
%------------------------------------------------
\subsection{Paramatric and Non-paramatric Model:}
\begin{frame}
	\frametitle{Formal definition of k-NN:}
	\begin{block}{Paramatric Model:}
		 The model has a fixed mumber of parameters.
	\end{block}

	\begin{block}{Non-paramatric Model:}
	The mumber of parameters grow with the amount of training data.
	\end{block}	
	\begin{itemize}
		\item Paramatric Model:	faster to use, but has stronger assumptions on the data distribution.
		\item Non-paramatric Model: more flexible, but higher computational cost. E.g., k-NN
	\end{itemize}
\end{frame}
%------------------------------------------------
%------------------------------------------------
...
\subsection{Examples of kNN}
\begin{frame}
	\frametitle{Examples of kNN, k = 3}
	%\begin{block}{Examples of Feature Vectors}
	\begin{figure}[h]
		\centering
		\includegraphics[scale=0.45]{fig1-14a.pdf}			
	\end{figure}
	%\end{block}
	%Do
\end{frame}
...
\begin{frame}
	...
	\begin{figure}[h]
		\centering
		\includegraphics[scale=0.3]{EuclideanDistance.png}		
	\end{figure}
	%\end{block}
	%Do
\end{frame}
...
%\subsection{Distance function}
\begin{frame}
	\frametitle{p = infinity}
	%\begin{block}{Examples of Feature Vectors}
	\begin{figure}[h]
		\centering
		\includegraphics[scale=0.3]{ChebyshevDistance.png}		
	\end{figure}
	%\end{block}
	%Do
\end{frame}
...
%\subsection{Distance function}
\begin{frame}
	\frametitle{p = 1,2,...,infinity}
	%\begin{block}{Examples of Feature Vectors}
	\begin{figure}[h]
		\centering
		\includegraphics[scale=0.3]{MinkowskiDistance.png}		
	\end{figure}
	%\end{block}
	%Do
\end{frame}
...
%\subsection{Distance function}
\begin{frame}
	\frametitle{Other distance}
	%\begin{block}{Examples of Feature Vectors}
	\begin{figure}[h]
		\centering
		\includegraphics[scale=0.3]{HammingDistance.png}		
	\end{figure}
	%\end{block}
	%Do
\end{frame}
...
\begin{frame}
	...
	\begin{figure}[h]
		\centering
		\includegraphics[scale=0.3]{CosineSimilarity.png}		
	\end{figure}
	%\end{block}
	%Do
\end{frame}
...
\begin{frame}
	...
	\begin{block}{How to choose a suitable K}
		A smaller K \\
		\begin{itemize}
		\item Reduce the approximation error of the learning [Training data]
		\item Enlarge the estimation error of the learning [Test data]
		\item A more complex model, easy to overfitting
	\end{itemize}
		
		A larger K:
		
		\begin{itemize}
			\item Reduce the estimation error of the learning
			\item Enlarge the approximation error of the learning
...
\frametitle{The best K}
\begin{block}{How to choose a suitable K}
	The best choice of \textbf{k} depends upon the data. \\
	Generally, larger values of \textbf{k} reduces effect of the noise on the classification, but make boundaries between classes less distinct.\\
	A good \textbf{k} can be selected by various heuristic techniques (see hyperparameter optimization).
	
	In binary (two class) classification problems, it is helpful to choose \textbf{k} to be an odd number to avoid tie.
...
%\subsection{Examples of kNN}
\begin{frame}
	\frametitle{Examples of kNN, k = 1}
	%\begin{block}{Examples of Feature Vectors}
	\begin{figure}[h]
		\centering
		\includegraphics[scale=0.35]{fig1-14b.pdf}		
	\end{figure}
	%\end{block}
	%Do
\end{frame}
...
\begin{frame}
	...
	\begin{figure}[h]
		\centering
		\includegraphics[scale=0.5]{BayesOPT.png}
		%\caption{ BayesOPT .}
	\end{figure}
\end{frame}
%------------------------------------------------
\begin{frame}
\frametitle{1-NN Convergence}
\textbf{[Cover, Hart, 1967]}~\\
...
The k-nearest neighbour classifier can be viewed as assigning the \textbf{k} nearest neighbours a weight  \textbf{1/k} and all others 0 weight. \\~~\\
This can be generalised to weighted nearest neighbour classifiers. \\~~  \\
That is, where the $i$-th nearest neighbour is assigned a weight \textbf{$w_{ni}$},with $\sum_{i=1}^{n}{w_{ni}}=1$.\\~~ \\
...
\begin{frame}
	...
	\begin{figure}[h]
		\centering
		\includegraphics[scale=0.3]{cursePointHyperplane}
		%\caption{}
	\end{figure}
...
For machine learning algorithms, this is highly relevant. As we will see later on, many classifiers (e.g. the \textbf{Perceptron} or \textbf{SVMs}) place hyper planes between concentrations of different classes. \\
One consequence of the curse of dimensionality is that most data points tend to be very close to these hyperplanes and it is often possible to perturb input slightly (and often imperceptibly) in order to change a classification outcome. This practice has recently become known as the creation of \textbf{adversarial samples}, whose existents is often falsely attributed to the complexity of neural networks.

\end{frame}

%------------------------------------------------
\subsection{Data with low dimensional structure}
\begin{frame}
	\frametitle{Data with low dimensional structure}
	However, not all is lost. Data may lie in low dimensional subspace or on sub-manifolds. Example: natural images (digits, faces). \\
	...
	\begin{figure}[h]
		\centering
		\includegraphics[scale=0.3]{LowDimData.png}
		\caption{An example of a data set in 3d that is drawn from an underlying 2-dimensional manifold. The blue points are confined to the pink surface area, which is embedded in a 3-dimensional ambient space.}
	\end{figure}
...
\begin{block}{Data reduction}
	...
	\begin{enumerate}[1]
		...
		\item Separate the rest of the data into two sets: \\
		(i) the prototypes that are used for the classification decisions and \\
		(ii) the absorbed points that can be correctly classified by k-NN using prototypes. \\
...
\begin{block}{Cons}
	\begin{itemize}
		...
		\item Prediction stage might be slow (in O(N))
...
\bibitem{1NN} Cover, Thomas, and, Hart, Peter. Nearest neighbor pattern classification. IEEE Transactions on Information Theory, 1967, 13(1): 21-27.
\end{thebibliography}
\end{frame}
